...
('Cliente1', 'pass1', 'Mario', 'Rossi', 'mario.rossi@example.com', 'Cliente'),
('Cliente2', 'pass2', 'Luigi', 'Verdi', 'luigi.verdi@example.com', 'Cliente'),
...
INSERT INTO Ordine (DataOrdine, Stato, DataSpedizione, Username, Saldo) VALUES
('2023-06-15', 'Consegnato', '2023-06-20', 'Cliente1', 499.99);

INSERT INTO Ordine (DataOrdine, Stato, Username, Saldo) VALUES
('2023-07-05', 'In lavorazione', 'Cliente1', 279.99);
...
CREATE TRIGGER trg_cambioStatoOrdineSenzaVerificaPagamento
    ...
    FOR EACH ROW
    BEGIN
        IF (NOT EXISTS(SELECT DataPagamento FROM pagamento WHERE NumeroOrdine = OLD.NumeroOrdine) && NEW.Stato = 'Spedito') THEN
...
BEGIN
    START transaction;
       ...
       SELECT Valore*inputNumero INTO @salDoPrimoSet FROM setlego WHERE CodiceSet = inputCodiceSet;
       INSERT INTO Ordine (NumeroOrdine, DataOrdine, Stato, Username, Saldo) VALUES (inputNumeroOrdine,@dataOrdine,'In lavorazione',inputUsername, @salDoPrimoSet);
...
BEGIN
    START transaction;
    SELECT Saldo INTO @saldoAttuale FROM ordine WHERE NumeroOrdine = inputNumeroOrdine;
    SELECT Valore INTO @valoreNuovoSet FROM setLego WHERE CodiceSet = inputCodiceSet;
...
BEGIN
    SELECT ordine.NumeroOrdine, ordine.DataOrdine, ordine.Stato, ordine.Saldo, ordine.DataSpedizione FROM ordine
...
BEGIN
    SELECT DettagliOrdine.CodiceSet, DettagliOrdine.Numero, SetLego.Collezione, SetLego.Anno FROM dettagliordine
    INNER JOIN setlego USING(CodiceSet)
...
/* SP: admin vuole poter cambiare lo stato di un ordine da Spedito a Consegnato*/
DELIMITER $$
