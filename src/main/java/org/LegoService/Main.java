...
//----Azioni Cliente----//
    private static void aggiungiSetLegoMiei(Database db, infoCredenziali parametriUtente){
        ...
        List<Integer> possiede = db.sql("call sp_cheSetLegoHo(?)",Username).results(Integer.class);
    ...
    private static void vediMieiOrdini(Database db, infoCredenziali parametriUtente){
        ...
        List<mieiOrdini> ordine = db.sql("call sp_vediMieiOrdini(?)",Username).results(mieiOrdini.class);
        AsciiTable at = new AsciiTable();
        at.addRule();
        at.addRow("Numero ordine","Data Ordine","Data Spedizione","Stato");
        at.addRule();
        for (mieiOrdini indicatore:ordine){
            if(indicatore.DataSpedizione == null){at.addRow(indicatore.NumeroOrdine,indicatore.DataOrdine,"null",indicatore.Stato);}
            else{at.addRow(indicatore.NumeroOrdine,indicatore.DataOrdine,indicatore.DataSpedizione,indicatore.Stato);}
        }
        at.addRule();
        System.out.println(at.render());
    ...
    private static void rimuoviDaMailingList(Database db, infoCredenziali parametriUtente){
        ...
        while(verifica == true) {
            List<String> partecipazione = db.sql("call sp_aCheMailingListSonoIscritto(?)",Username).results(String.class);
            AsciiTable at = new AsciiTable();
            at.addRule();
            at.addRow("Nome mailinglist");
            at.addRule();
    ...
    private static void ordineNonPagato(Database db, infoCredenziali parametriUtente){
        ...
        List<ordineNonPagati> ordine = db.sql("call sp_ordineNonPagato()").results(ordineNonPagati.class);
